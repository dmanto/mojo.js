...
class TestClient extends MockClient {
  ...
  async sendOk (data) {
    await new Promise(resolve => this.res.send(data, resolve));
    this.assert('ok', [true], 'send message', this.sendOk);
  }

  async websocketOk (url) {
    this.res = await this.websocket(url);
    this._finished = null;
    this.res.on('close', (...args) => (this._finished = args));
    this._messages = on(this.res, 'message');
    await new Promise(resolve => this.res.once('open', resolve));
  ...
  _waitFinished () {
    ...
    return new Promise(resolve => this.res.once('close', resolve));
  }
}
